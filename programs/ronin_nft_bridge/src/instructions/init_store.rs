...
pub struct InitStore<'info> {
    ...
    // Optional: PDA for LzReceiveTypes, used by Executor
    #[account(
        init,
        payer = payer,
...
impl InitStore<'_> {
    pub fn apply(ctx: &mut Context<InitStore>, params: &InitStoreParams) -> Result<()> {
        // Enforce: init can be called only by the intended admin wallet
        require_keys_eq!(ctx.accounts.payer.key(), params.admin, OAppError::InvalidTokenDestination);
