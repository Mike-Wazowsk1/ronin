...
async function fetchEvmOappState(eid: EndpointId, hre: HardhatRuntimeEnvironment, contractName: string) {
    ...
    let data: unknown = 'N/A'
    try {
        // Try optional view method 'data()' (present in string OApp example)
    ...
    return { address: contract.address, data }
}
...
async function fetchSolanaOappState(eid: EndpointId) {
    ...
    const client = new myoapp.RoninNftBridge(publicKey(deployment.programId))
...
task('lz:oapp:get', 'Fetch and pretty-print OApp state for each configured endpoint', action)
    ...
    .addOptionalParam('contractName', 'Name of the EVM OApp contract (default: RoninNftBridge)', 'RoninNftBridge', types.string)
